...
# ================= CÁMARA =================
cap = cv2.VideoCapture(0)  # Webcam actual

# --- Cámara industrial futura (dejar comentado) ---
# cap = cv2.VideoCapture(1)  # Ajustar según dispositivo USB
...
for i in range(NUM_CARRILES):
    x_min = i * ANCHO_CARRIL
    x_max = (i + 1) * ANCHO_CARRIL
    CARRILES.append((x_min, x_max))
...
while True:
    ...
    for det in detections:
        ...
        for i, (xmin, xmax) in enumerate(CARRILES):
            if xmin <= cx < xmax:
                carril_detectado = i
                break
    ...
    cv2.imshow("Sistema Multi-Carril", frame)
